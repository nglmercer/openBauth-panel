...
function prepareFetchOptions(
  ...
  const {
    useProxy = proxyConfig.isEnabled(),
...
const http = {
  get: <T>(url: string, options: FetchOptions = {}): Promise<T> => {
    const [finalUrl, finalOptions] = prepareFetchOptions(url, {
      ...options,
      method: "GET",
    });
  ...
  post: <T>(
    ...
    const requestWithoutProxy =
      options.useProxy !== false ? () => fetch(url, finalOptions) : undefined;
