...
auth.post("/forgot-password", async (c) => {
  try {
    ...
    if (user) {
      ...
      // For now and tests, it is stored in the DB.
      if (services.notificationService && process.env.NODE_ENV !== 'test') {
...
auth.post("/reset-password", async (c) => {
  try {
    ...
    const result = await userController.update(verification.userId, {
...
auth.post("/verify-email", async (c) => {
  try {
    ...
    const result = await (services.authService as any).updateUser(verification.userId, {
      is_active: true
    });
    ...
    await services.auditService.logAuthEvent('user.email.verified', verification.userId, c.req.header('x-forwarded-for') || 'unknown');
