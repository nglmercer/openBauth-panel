...
export function validateFromSchemas(
  schemas: TableSchema[],
): Record<string, { create: z.ZodObject<any>; update: z.ZodObject<any>; read: z.ZodObject<any> }> {
  const validators: Record<string, { create: z.ZodObject<any>; update: z.ZodObject<any>; read: z.ZodObject<any> }> = {};

  for (const tableSchema of schemas) {
    validators[tableSchema.tableName] = {
      create: createZodCreateValidatorFromTableSchema(tableSchema),
      update: createZodUpdateValidatorFromTableSchema(tableSchema),
      read: createZodValidatorFromTableSchema(tableSchema)
    };
  }
